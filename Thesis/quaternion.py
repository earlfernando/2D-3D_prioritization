...
def quaterinion_multiplication(v,s):

    inverse_r1 = np.linalg.inv(v)
    inner_product = np.matmul(inverse_r1,s)
    inverse_inner_product = np.linalg.inv(inner_product)
    trace_inv_in_prod = np.trace(inverse_inner_product)
    cos_theta = trace_inv_in_prod/2
    theta = math.acos(cos_theta)

    return  theta

def inner_product_calculation(r,s):
    out= np.matmul(r,s)
...
def mat_quat_mul(v,s):
    inverse_r1 = [np.linalg.inv(local)for local in v]
    inner_product = [inner_product_calculation(i,j)for i,j in zip(inverse_r1,s)]
    print(inner_product)
