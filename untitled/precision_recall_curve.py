...
file_name_random_forest = "/home/earl/Thesis/GreatCourt/test_model_random_forest_10000.fpForest"
...
def random_forest_chunks(headers, feature_length, csv_file_location, file_name):
    ...
    clf = rerfClassifier(projection_matrix="RerF",n_estimators =10, max_depth =10, max_features= None,oob_score = True, n_jobs =-1, random_state=42,feature_combinations=1)
...
def prediction_forest(headers, feature_length, csv_file_location_test, file_name_random_forest, clf, file_name,
            ...
            #### precision recall
                ...
                if truth == 0:
                    ...
                    if truth == classified:
                        ...
                        model_accuracy += 1
        # print(prob/10,'positive',classified,
    ...
    # print(prob/10,'negative',classified,truth,np.argmax(prob))
    print(model_accuracy / total, total)
    false_negative = np.subtract(local_negative,negative)
    false_positive = np.subtract(local_positive,positive)

    print(false_negative,negative,positive,false_positive,local_positive,local_negative)
    recall_down =np.add(positive,false_negative)
    precision_down = np.add(positive,false_positive)
    precision =[]
    recall =[]
    for i in range (number_axis):
        if precision_down:
            precision.append(positive[i]/precision_down[i])
        else:
            precision.append(0)
        if recall_down:
            recall.append(positive[i],recall_down[i])
        else:
            recall.append(0)
    print(precision,recall)
    fig, ax = plt.subplots()
    line = ax.plot(recall,precision, label='precision-recall curve')
    ax.legend()
    plt.xlabel('recall')
...
prediction_forest(headers, feature_length, csv_file_location_kmeans_test, file_name_random_forest, clf,
                  file_name=file_name_random_forest, selected_col=selected_columns)
